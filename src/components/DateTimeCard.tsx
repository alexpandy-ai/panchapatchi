...
import { formatBilingual, UI, weekdayBilingual } from "../utils/bilingual";
...
export function DateTimeCard({ value, onChange }: DateTimeCardProps) {
  ...
  const listId = useId();
  const weekdayBi = weekdayBilingual(value.getDay());
  const [locationInput, setLocationInput] = useState(() => formatBilingual(locationDisplay));
  const [latInput, setLatInput] = useState("");
  const [lngInput, setLngInput] = useState("");
  const draftDirtyRef = useRef(false);

  useEffect(() => {
    setLocationInput(formatBilingual(locationDisplay));
  }, [locationDisplay]);
  ...
  return (
    <section className="datetime-card" aria-label={`${UI.dateTime.ta} ${UI.dateTime.en}`}>
      <div className="datetime-card__row">
        ...
        <label className="datetime-field datetime-field--location">
          ...
          <input
            ...
            placeholder={`${UI.location.ta} / ${UI.location.en}`}
            aria-label={`${UI.location.ta} ${UI.location.en}`}
            autoComplete="off"
          />
          <datalist id={listId}>
          ...
          <div className="datetime-field__coords-row">
            <label className="datetime-field__coord">
              ...
              <input
                ...
                placeholder="13.08"
                aria-label={`${UI.latitude.ta} ${UI.latitude.en}`}
                autoComplete="off"
              />
            </label>
            <label className="datetime-field__coord">
              ...
              <input
                ...
                placeholder="80.27"
                aria-label={`${UI.longitude.ta} ${UI.longitude.en}`}
                autoComplete="off"
              />
            </label>
          </div>
        ...
        <button
          ...
          aria-label={`${submitLabel.ta} ${submitLabel.en}`}
        >
